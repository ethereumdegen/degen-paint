...
```
                       ┌──────────────────────────────────────────┐
   agent ──── stdio ──▶│ atelier-mcp   (MCP tools from registry)   │
                       ├──────────────────────────────────────────┤
   agent ──── argv ───▶│ atelier-cli   (`atl`, verbs from registry)│
                       ├──────────────────────────────────────────┤
   human ──── webview ▶│ apps/studio   (Tauri v2 + Vite/TS UI)     │
                  ...
                  └───────┬──┘ └─────┬────┘ └───┬──────┘ └────┬─────┘
                          └──────────┴─────┬────┴─────────────┘
                              ┌────────────▼───────────┐
                              │ atelier-render         │  PNG JPG WebP AVIF TIFF
                              │ atelier-inspect        │  SVG PDF GLB
                              └────────────────────────┘  + digest / lint / diff
```

**The invariant that matters:** there is exactly one engine. The pixels a human sees in the Tauri
viewport and the pixels an agent gets from `atl render` come from the same Rust code path. No
...
|---|---|---|
| `atelier-core` | Document model (serde), ids & selectors, geometry, color, units/DPI, content-addressed asset store, op registry, JSON-patch journal, undo/redo, project load/save, JSON Schema export | `serde`, `serde_json`, `schemars`, `kurbo`, `palette`, `blake3`, `indexmap` |
| `atelier-raster` | Layer compositor, blend modes, masks, clipping, adjustment layers, filters, selections, brush/stroke replay | `tiny-skia`, `image`, `imageproc`, `fast_image_resize`, `rayon` |
| `atelier-vector` | Path model, boolean ops, offset/outline/simplify, gradients, text shaping → outlines, SVG parse/serialize | `kurbo`, `i_overlay`, `rustybuzz`, `fontdb`, `ttf-parser`, `usvg`, `svgtypes` |
| `atelier-model3d` | glTF authoring: primitives, extrude/revolve/loft, PBR materials, node graph, TRS animation, validation | `gltf-json`, `lyon_tessellation`, `mikktspace`, `meshopt` |
| `atelier-ai` | Optional provider layer: fal.ai, QuiverAI; key resolution, caching, provenance, budget | `reqwest`, `tokio`, `keyring`, `secrecy`, `eventsource-stream` |
| `atelier-render` | Unified render targets, scale/DPI, thumbnails, wgpu PBR renderer (offscreen + on-surface) | `wgpu`, `oxipng`, `zune-jpeg`, `resvg` |
| `atelier-inspect` | Digest, measurement, lint rules, SSIM/ΔE diff, annotate overlay | `dssim-core`, `image-compare` |
| `atelier-cli` | `atl` binary; subcommands generated from the registry | `clap`, `indicatif` |
| `atelier-mcp` | MCP server over stdio; tools generated from the registry | `rmcp` |
| `atelier-wasm` | `wasm-bindgen` surface for the browser build | `wasm-bindgen`, `js-sys` |
...
```
  atl op raster.filter.gaussian-blur --layer '#sky' --radius 12
        │
        ├─ 1. parse argv against the op's JSON Schema        (atelier-cli)
        ├─ 2. load project.json + open asset store           (atelier-core)
        ├─ 3. snapshot the affected document                 (atelier-core)
        ├─ 4. resolve the selector '#sky' → LayerId          (atelier-core)
        ├─ 5. apply(&mut Project, args)                      (atelier-raster)
        │       └─ mutates document JSON only; pixel results
        │          are written to the asset store by hash
        ├─ 6. diff snapshot → RFC-6902 patch                 (atelier-core)
        ├─ 7. append {op, args, patch, ts} to history.jsonl  (atelier-core)
        ├─ 8. write project.json atomically                  (atelier-core)
        └─ 9. emit OpEffect as JSON on stdout                (atelier-cli)
...
- `atl gc` prunes blobs unreachable from the document set and the journal.
...
Capabilities are reported, never assumed: `atl doctor` and the GUI's capability probe both report
