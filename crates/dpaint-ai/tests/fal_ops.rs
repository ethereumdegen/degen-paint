...
fn generate_submits_polls_to_completion_and_lands_a_layer_with_provenance() {
    ...
    assert_eq!(p.prompt.as_deref(), Some("storm light over a wheat field, 35mm"));
...
fn the_model_id_comes_from_configuration_and_can_be_overridden_per_call() {
    ...
    assert_eq!(transport.calls()[0].url, "https://queue.fal.run/fal-ai/recraft-v3");
    assert_eq!(
        fx.raster().layers.last().unwrap().provenance.as_ref().unwrap().model,
        "fal-ai/recraft-v3"
    );
}
...
fn inpaint_sends_the_documents_current_selection_as_the_mask() {
    ...
    assert_eq!(mask.get_pixel(8, 8).0[0], 255, "the selected quadrant is paintable");
...
fn inpaint_without_a_selection_fails_before_any_request_is_made() {
    let transport = Arc::new(fal_job("fal-ai/flux-general/inpainting", png(8, 8, [0, 0, 0, 255])));
    let mut fx = Fixture::new(transport.clone(), fal_keys());
...
fn remove_background_turns_the_cutout_into_a_layer_mask() {
    ...
    let layer = fx.raster().layer(&dpaint_core::LayerId::from(SRC_LAYER)).unwrap();
    ...
    assert_eq!(fx.assets().get(asset).unwrap(), png(32, 32, [20, 60, 120, 255]));

    let m = image::load_from_memory(&fx.assets().get(&mask.asset).unwrap())
        .unwrap()
        .to_rgba8();
    assert_eq!(m.get_pixel(4, 4).0[0], 255, "subject side is opaque in the mask");
...
fn upscale_replaces_the_blob_and_keeps_the_layer_the_same_size_on_canvas() {
    let transport = Arc::new(fal_job("fal-ai/clarity-upscaler", png(64, 64, [7, 7, 7, 255])));
    let mut fx = Fixture::new(transport.clone(), fal_keys());
    let before = match &fx.raster().layer(&dpaint_core::LayerId::from(SRC_LAYER)).unwrap().kind {
...
fn outpaint_grows_the_canvas_and_shifts_existing_content() {
    ...
    let src = fx.raster().layer(&dpaint_core::LayerId::from(SRC_LAYER)).unwrap();
...
fn texture_generate_binds_editable_raster_documents_to_the_material() {
    ...
    fx.engine.workspace.project.add_document(Document::Model(model));
...
fn a_provider_failure_leaves_the_project_untouched() {
    ...
    let transport = Arc::new(
        dpaint_ai::RecordedTransport::new().on(
            Method::Post,
            "queue.fal.run",
    ...
    let err = fx
        .engine
        .apply("ai.image.generate", json!({"prompt": "a barn"}), None, false)
        .unwrap_err();
